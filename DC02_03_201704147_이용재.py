...
def dominant(frame_rate, chunk):  # decode_file, listen_linux 함수에서 쓰임 // 수신한 청크의 주파수를 dominant() 함수로 계산 // 푸리에 트랜스폼 즉 소리를 주파수로 변환하기 위해 사용
    w = np.fft.fft(chunk)
    freqs = np.fft.fftfreq(len(chunk))  # 실수로부터 주파수를 얻기 위해 사용 (numpy 씀)
    peak_coeff = np.argmax(np.abs(w))  # 가장 크고 특징적인 주파수를 출력 함
...
def extract_packet(freqs):  # listen_linux 함수에서 쓰임
    ...
    # 이것도 람다임
    # for f in freqs:
    #     round((f - START_HZ) / STEP_HZ)) 똑같은 주파수가 두개 나옴
    bit_chunks = [c for c in bit_chunks[1:] if 0 <= c < (2 ** BITS)]  ## BITS가 여기에 // 이거는 반대로 해서 원래가 뭔지 알아내는 것 // 톤이 1비트에 4비트므로 그이상이되면 잘못된 데이터이므로 짤라줌
...
# 1톤에 4비트식 있으므로 그걸 아스키로 변환하면 됨 따라서 이걸 decode_bitchunks에서 함
# ** 원래 1톤이 2비트로 이루어져서 0-3까지밖에 안나옴
# 하지만 1-release는 4비트이므로 0-15까지 나옴
# 이렇게 접근하는거였음!!
...
def listen_linux(frame_rate=44100, interval=0.1):  # 사실상 main함수로 봐야 함
    ...
    mic.setchannels(1)  # 주파수대역을 어느정도 쓰냐
    mic.setrate(44100)  # 샘플링
    mic.setformat(alsaaudio.PCM_FORMAT_S16_LE)  # 마이크 객체 생성 **PCM이 여기서나옴!!!!

    num_frames = int(round((interval / 2) * frame_rate))  # 1초에 몇번 샘플링, 어느 간격으로 샘플링 할 건지
    ...
    while True:
        ...
        if in_packet and match(dom, HANDSHAKE_END_HZ):  # match() // (두 수의 차의 절대값 구하는 과정) 핸드쉐이크 종료 주파수를 수신하고 디코딩을 시작
            byte_stream = extract_packet(packet)  # extract_packet() [ decode_bitchunks() ]  주파수를
            try:
            ...
            packet = []
            in_packet = False
        elif in_packet:  # 여기서는 주파수를 계속 받기만 함
            packet.append(dom)
        elif match(dom, HANDSHAKE_START_HZ):  # match() // 두 수의 차의 절대값 구하는 과정 / 마지막에 패킷을 읽을 준비를 함
            in_packet = True
